...
import { useActions, type ModalPayload, type ModalState } from '../../contexts/ActionsContext'
    ...
    >
      ...
      {modal.type === 'maintenance' && (
        <>
          <label>
            Tài sản
            <select value={form.assetId} onChange={(event) => setField('assetId', event.target.value)} required>
      ...
      {modal.type === 'transfer' && (
        <>
          ...
          <label>
            Nhân viên nhận
            <select value={form.toEmployeeId} onChange={(event) => setField('toEmployeeId', event.target.value)}>
