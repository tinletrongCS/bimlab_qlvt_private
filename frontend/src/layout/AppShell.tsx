...
const NAV_ITEMS: NavItem[] = [
  ...
  { to: '/subscriptions', label: 'Subscription', icon: <FiCreditCard />, permission: 'subscription_manage' },
  { to: '/vendors', label: 'Nhà cung cấp', icon: <FiBriefcase />, permission: 'vendor_manage' },
  { to: '/requests', label: 'Đề nghị mua sắm', icon: <FiShoppingCart />, permission: 'purchase_request_create' },
...
export function AppShell() {
  ...
  const visibleItems = useMemo(() => NAV_ITEMS.filter((item) => hasPermission(item.permission)), [hasPermission])
    ...
    <main className="app-shell">
      <aside className="sidebar">
        ...
        <nav>
          {visibleItems.map((item) => (
            <NavLink to={item.to} key={item.to} className={({ isActive }) => (isActive ? 'active' : '')}>
              {item.icon}
              {item.label}
            </NavLink>
          ))}
        ...
        <div className="user-card">
          ...
          <button className="logout-button" onClick={() => void logout()} disabled={authSubmitting} title="Đăng xuất">
            <FiLogOut />
          </button>
        </div>
      </aside>
      ...
      <section className="content">
        <header className="topbar">
          ...
          <button className="secondary" onClick={() => void refresh()} disabled={loading || actionSubmitting}>
            <FiRefreshCw /> Làm mới
          </button>
        </header>
