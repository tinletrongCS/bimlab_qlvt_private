...
export function AssetCategoriesPage() {
  ...
  const importPreviewRows = useMemo(() => {
    ...
    else if (importPreviewFilter === "INVALID") rows = rows.filter((r) => r.status === "INVALID" || r.status === "HAS_ERROR");
    else if (importPreviewFilter === "WARNING") rows = rows.filter((r) => r.warnings && r.warnings.length > 0);
    return rows;
