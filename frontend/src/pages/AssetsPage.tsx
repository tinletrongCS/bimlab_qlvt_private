...
export function AssetsPage() {
  ...
  return (
    <section className="panel">
      <PanelHeader title="Danh sách tài sản" action={canManage} onAdd={() => openModal({ type: 'asset', mode: 'create' })} />
      <DataTable
        data={assets}
        ...
        columns={[
          ...
          { key: 'vendor', title: 'Nhà cung cấp', render: (item) => item.vendor?.name || '—' },
          {
            key: 'owner',
            title: 'Người dùng',
            render: (item) => <span className="muted-cell">{employeeName(item.assignedEmployeeId)}</span>,
          },
          {
            key: 'scope',
            title: 'Liên kết',
            render: (item) => (
              <span className="muted-cell">
                {departmentName(item.departmentId)} · {siteName(item.siteId)} · {projectName(item.projectId)}
              </span>
            ),
          },
