    ...
    stage('Smoke: run + healthcheck') {
      steps {
        sh '''
          ...
          docker exec "$PG" psql -U smoke -d bimlab -v ON_ERROR_STOP=1 -c 'CREATE SCHEMA hrm; CREATE TABLE hrm.employees (id bigint PRIMARY KEY, full_name varchar(255)); CREATE SCHEMA auth; CREATE TABLE auth.users (username varchar(100) PRIMARY KEY, full_name varchar(255));' >/dev/null
