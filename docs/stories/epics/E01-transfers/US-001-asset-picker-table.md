# US-001 Bộ chọn tài sản dạng bảng checkbox với lọc danh mục cho phiếu bàn giao
...
2. Lọc tài sản theo danh mục (Asset Category).
...
- [x] Bộ chọn có ô tìm kiếm (mã/tên tài sản) và dropdown chọn danh mục từ API `loadAssetCategories`.
- [x] Có checkbox chọn tất cả ở header và checkbox từng dòng tài sản.
- [x] Hiển thị các thông tin: STT, Checkbox, Mã tài sản, Tên tài sản, Danh mục, Trạng thái, Vị trí hiện tại.
...
- [x] Chạy qua kiểm tra linter (`corepack pnpm lint`) và build (`corepack pnpm build`).

## Design Notes

- Commands: N/A (Frontend only)
- Queries: `loadAssetCategories()`
- API: Dùng API hiện có
...
- `corepack pnpm --dir frontend build`: Production bundle built cleanly with Vite & TypeScript in 2.64s.
